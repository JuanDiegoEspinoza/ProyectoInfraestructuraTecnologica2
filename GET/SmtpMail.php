...
if(!defined("SMTPPORT"))	 {define("SMTPPORT",25); }			// Puerto
if(!defined("SMTPLANGUAGE")) {define("SMTPLANGUAGE","es"); }	// Idioma es=Espa�ol / en=Ingles
...
class SmtpMail {
	function SmtpMail ($SmtpServer, $SmtpPort, $SmtpUser, $SmtpPass, $from, $to, $cc, $bcc, $subject, $body) {
		...
		$this->SmtpSendUserName	 = base64_encode ($SmtpUser);
		...
		$this->fault			 = "";
		$this->request			 = "";
		$this->CheckPatameters("SmtpServer,SmtpSendUserName,SmtpFrom,SmtpTo,SmtpHtmlBody");
	}
	...
	function SendMail(){
		...
		// Iniciando secuencia
		$this->SmtpAsign("HELO ".$this->SmtpServer.CRLF);
